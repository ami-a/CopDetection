...
def custom_get_detection_array(
    ...
    #convert coordinates format to (xmin,ymin,width,height)
    output_dict['new_coordinates']=list(
        map(lambda x:get_box_coordinates(x,image.shape),output_dict['detection_boxes'])
    )
    #build the detection_array
    output= [
        [output_dict['detection_scores'][i],output_dict['new_coordinates'][i]]
            ...
            and output_dict['new_coordinates'][i][2]>0 #make sure width>0
            and output_dict['new_coordinates'][i][3]>0 #make sure height>0
    ]
    #print(output)
    return output
...
def get_det_vars(det_model_path):
    ...
    return DetectionVars(
        detection_model=get_model(det_model_path),#custom loading the detection model
